...
public class PlayerThreadListen extends Thread
{
	PlayerOnline player;
	private int countBuffer=0;
	
   	public PlayerThreadListen(PlayerOnline p) {
		player=p;
	}
			...
			try {
				...
				{ 
					...
					case "Monster":
						...
						break;
					case "TowersGeneral":
						String[] splitGeneral=split[1].split(":");
						System.out.println("mando "+splitGeneral[0]+" "+Double.parseDouble(splitGeneral[1])+" "+ Integer.parseInt(splitGeneral[2])+" "+ Integer.parseInt(splitGeneral[3]));
						player.addToGeneralInfo(new TowerInfo(splitGeneral[0],Double.parseDouble(splitGeneral[1]), Integer.parseInt(splitGeneral[2]), Integer.parseInt(splitGeneral[3])));
					case "Tower":
